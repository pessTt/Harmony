...
let screenStream = null; // stream de tela (vídeo + áudio do sistema, se disponível)
let screenOn = false;

const peers = new Map(); // peerId -> { pc, username, avatar, tileEl, videoEl, audioEl, memberEl, volume }
...
// ===================== PREFERÊNCIAS SALVAS (servidor, avatar) =====================
...
// Reduz a imagem escolhida para um quadrado pequeno (evita mandar fotos gigantes pelo socket)
...
function setupSocketListeners() {
  ...
  socket.on('chat-message', ({ id, username: uname, message }) => {
    if (id === socket.id) return; // já mostramos a nossa própria mensagem na hora
...
function createPeerConnection(peerId, uname, avatar) {
  ...
  pc.ontrack = (e) => {
    ...
    if (!peer) return;
    // Se por algum motivo o navegador não agrupar a track num stream,
    // criamos um stream avulso só com essa track.
...
function createVideoTile(peerId, uname, avatar) {
  ...
  if (peerId !== 'self') {
    ...
    tile.querySelector('.volume-row').remove(); // não precisa controlar o volume da própria transmissão
  }
...
// Tenta capturar em 720p/60fps COM áudio do sistema; se não der (ex.: janela
// específica, ou plataforma sem suporte), cai pra qualidades/combinações
// mais simples em vez de simplesmente falhar.
async function getScreenStreamWithFallback(sourceId) {
  const videoTiers = [
    ...
    {}, // último caso: sem exigências, usa o padrão do sistema
  ];
  let lastError;
...
// Sincroniza (adiciona/troca/remove) as tracks de vídeo E áudio da tela
// compartilhada em TODOS os peers conectados. Sempre passa o MediaStream de
// verdade como segundo argumento de addTrack (nunca a própria track).
